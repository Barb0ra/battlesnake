...
def generate_possible_states(game_state, my_action):
  ...
  for move_combination in move_combinations:
    ...
    for snake_index, move in enumerate(move_combination):
      next_state = next_state_for_action(next_state, snake_index + 1, move)
...
class StateNode:
  ...
  def __init__(self, game_state, parent=None):
    ...
    self.terminal = is_dead(game_state,
                            0)[0] or len(game_state['snake_heads']) <= 1
  ...
  def update_value(self, value):
    self.n_visited += 1
    self.value = (self.value * (self.n_visited - 1) + value) / self.n_visited
