...
def load_game_state_data():
    ...
    for line in data:
        ...
        values.append(split_line[7])
        rewards.append(split_line[8])
...
def train_gp(state_vectors, td_lambda_targets):
...
def learn_first_batch(model_name, games_per_batch, sampling_rate):
    ...
    # train a Gaussian Process to predict the td_lambda_targets
    gp_value_fucntion = train_gp(state_vectors, td_lambda_targets)
    # save the data
    np.save('learning/value_functions/state_vectors.npy', state_vectors)
    np.save('learning/value_functions/td_lambda_targets.npy', td_lambda_targets)
    # save the Gaussian Process
    joblib.dump(gp_value_fucntion, model_name)
    return gp_value_fucntion


def learn_batch(model_name, games_per_batch, sampling_rate):
    # load the old data
    state_vectors = np.load('learning/value_functions/state_vectors.npy')
    td_lambda_targets = np.load(
        'learning/value_functions/td_lambda_targets.npy')
    # play a game
    snakes = [{'name': 'Bayesian', 'type': 'tree_search', 'port': 8000, 'params': [2.0, 8.0, 6.0, 0.9,
                                                                                   'lcb', True, 'basic']}, {'name': 'Baseline', 'type': 'one_step_lookahead', 'port': 8080, 'params': []}]
    run_games(games_per_batch, snakes, GO_FILE_PATH, 'game_log.txt', True)
...
def learn_value_function(model_name, num_batches, games_per_batch, sampling_rate):
    # learn the value function
    delete_old_game_state_data()
    gp_value_fucntion = learn_first_batch(
        model_name, games_per_batch, sampling_rate)
